...
const ExperienceCards = ({ theme, experiences = [] }) => {
  ...
  return (
    <div className="space-y-6">
      {experiences.map((exp, index) => {
        const id = exp.id ?? index;
        const company = exp.company;
        const position = exp.position || exp.role;
        const duration = exp.duration || exp.dates;
        const location = exp.location;
        const impacts = Array.isArray(exp.impacts) && exp.impacts.length > 0 ? exp.impacts : (exp.impact ? [exp.impact] : []);
        const description = exp.description || exp.detailedDescription;
        const technologies = exp.technologies || exp.skills || [];
        const image = getImage(exp);
        ...
        return (
          ...
          >
            ...
            <div className="p-8">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                ...
                <div className={`flex items-center gap-2 ${theme === "dark" ? "text-gray-400" : "text-gray-600"} text-sm`}>
                  <span>Click to {expandedCard === id ? 'collapse' : 'expand'}</span>
                  <ChevronDownIcon className={`h-5 w-5 transition-transform duration-300 ${expandedCard === id ? 'rotate-180' : ''}`} />
                </div>
              </div>

              {/* Impact Highlight as bullets */}
              {impacts.length > 0 && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className={`font-semibold ${theme === "dark" ? "text-yellow-300" : "text-yellow-700"} mb-1`}>
                        {impacts.length > 1 ? 'Key Impacts' : 'Key Impact'}
                      </h4>
            ...
            <motion.div
              initial={false}
              animate={{ height: expandedCard === id ? 'auto' : 0, opacity: expandedCard === id ? 1 : 0 }}
